...
document.addEventListener("DOMContentLoaded", () => {
    ...
    fetch(`/get-tipo-lead/?tipo=New Lead`)
    ...
    toggleInput.addEventListener("change", () => {
        ...
        fetch(`/get-tipo-lead/?tipo=${leadDisplay.textContent}`)
            .then(response => response.json())
            .then(data => console.log("Tipo de lead cargado:", data));
    });
    ...
    fetch("/get-programa/?tipo_lead_id=1")
        .then(response => response.json())
        .then(data => {
    ...
    fetch("/get-momento/?programa_id=1")
        .then(response => response.json())
        .then(data => {
            const modal = false;
    ...
    const submoments = () => {
        fetch("/get-submomento/?momento_id=1")
        .then(response => response.json())
        .then(data => {
        ...
        document.getElementById('tipo-lead-toggle').addEventListener('change', function () {
            ...
            fetch(`/get-programa/?tipo_lead_id=${tipoLeadId}`)
                .then(response => {
